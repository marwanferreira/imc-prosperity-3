...
class Trader:
    def run(self, state: TradingState):
        ...
        # Gestion intelligente de la conversion des MAGNIFICENT_MACARONS
        pos_macarons = state.position.get("MAGNIFICENT_MACARONS", 0)
        dynamic_conversions = {"MAGNIFICENT_MACARONS": {}}
        for possible_target in ["GOLDEN_BASKET", "SILVER_BASKET", "PLATINUM_BASKET"]:
            if possible_target in fair_values:
                dynamic_conversions["MAGNIFICENT_MACARONS"][possible_target] = {"conversionQuantity": 1}

        conv_decision = self.best_conversion("MAGNIFICENT_MACARONS", dynamic_conversions, fair_values, pos_macarons)
        conversions += conv_decision
    ...
    def best_conversion(self, product: str, conversions_available: Dict[str, Dict[str, Dict]], fair_values: Dict[str, float], position: int) -> int:
        best_gain = -float('inf')
        best_conversion = None
        if product in conversions_available:
            for target_product, conversion in conversions_available[product].items():
                target_qty = conversion["conversionQuantity"]
                target_price = fair_values.get(target_product, None)
                if target_price is not None:
                    estimated_gain = target_qty * target_price
                    if estimated_gain > best_gain:
                        best_gain = estimated_gain
                        best_conversion = target_product
        if best_conversion and abs(position) >= 10:
