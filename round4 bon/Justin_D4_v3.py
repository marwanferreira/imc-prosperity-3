...
class Trader:
    def run(self, state: TradingState):
        ...
        conv_decision = self.best_conversion("MAGNIFICENT_MACARONS", state, fair_values, pos_macarons)
    ...
    def best_conversion(self, product: str, state: TradingState, fair_values: Dict[str, float], position: int) -> int:
        best_gain = -float('inf')
        best_conversion = None
        if product in state.conversions_possible:
            for target_product, conversion in state.conversions_possible[product].items():
