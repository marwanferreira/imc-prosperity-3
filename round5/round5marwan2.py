...
# === Round 5: OwnTrade class and decoder ===
class OwnTrade:
    def __init__(self, symbol: str, price: int, quantity: int, counter_party: str = None):
        self.symbol = symbol
        self.price = price
...
BUG_BEHAVIOR_MAP = {
    "Amir": ["market_maker"],
    "Ayumi": ["momentum", "competitive"],
    "Ari": ["alpha-hunter"],
    "Anika": ["mean-reverter", "contrarian"],
    "Boris": ["hoarder", "noise"],
    "Bashir": ["risky"],
    "Bonnie": ["unpredictable"],
    "Blue": ["bearish"],
    "Sanjay": ["timing"],
    "Sami": ["HFT"],
    "Sierra": ["inefficient"],
    "Santiago": ["emotional"],
    "Mikhail": ["night", "illiquid"],
    "Mina": ["volume", "momentum"],
    "Morgan": ["risk-averse"],
    "Manuel": ["pattern"],
    "Carlos": ["resilient", "trend"],
    "Candice": ["mimic"],
    "Carson": ["mean-reverter"],
    "Cristiano": ["momentum", "emotional"],
}

def get_bug_tags(counter_party: str) -> List[str]:
    for name in BUG_BEHAVIOR_MAP:
        if name.lower() in counter_party.lower():
            return BUG_BEHAVIOR_MAP[name]
    return []

def get_recent_bug_tags(symbol: str, decoded_own_trades: Dict[str, List[OwnTrade]]) -> List[str]:
    if symbol not in decoded_own_trades:
        return []
    trades = decoded_own_trades[symbol]
    if not trades:
        return []
    # Look at last 3 trades with known bugs
    recent_tags = []
    for trade in reversed(trades[-3:]):
        if trade.counter_party:
            recent_tags += get_bug_tags(trade.counter_party)
    return recent_tags

def decode_own_trade(trade_obj):
    return OwnTrade(
        symbol=trade_obj.symbol,
        price=trade_obj.price,
        quantity=trade_obj.quantity,
        counter_party=getattr(trade_obj, "counter_party", None),
    )
...
class Trader:
    def run(self, state: TradingState):
        ...
        memory = {}
        decoded_own_trades = {
            symbol: [decode_own_trade(t) for t in trades]
        ...
        if "counterparty_stats" not in memory:
            memory["counterparty_stats"] = {}

        counterparty_stats = memory["counterparty_stats"]

        for symbol, trades in decoded_own_trades.items():
            for trade in trades:
                if trade.counter_party:
                    if trade.counter_party not in counterparty_stats:
                        counterparty_stats[trade.counter_party] = {
                            "total_trades": 0,
                            "buy_volume": 0,
                            "sell_volume": 0,
                        }
                    stats = counterparty_stats[trade.counter_party]
                    stats["total_trades"] += 1
                    if trade.quantity > 0:
                        stats["buy_volume"] += trade.quantity
                    else:
                        stats["sell_volume"] += abs(trade.quantity)

        for symbol, trades in decoded_own_trades.items():
            for trade in trades:
                tags = get_bug_tags(trade.counter_party or "")
                if "noise" in tags or "risky" in tags:
                    print(f"WARNING: Traded with {trade.counter_party} on {symbol} — tags: {tags}")
        ...
        memory["tick"] = state.timestamp
        if memory.get("skip_tick"):
            memory["skip_tick"] = False
            print(f"[SKIP TICK] Avoiding front-run or HFT on tick {state.timestamp}")
            return {}, 0, jsonpickle.encode(memory)
        fair_values = {}
        volatilities = {}
        regimes = {}
        momentums = {}
        ...
        memory["counterparty_stats"] = counterparty_stats
    ...
    def multi_level_market_make(self, product: str, order_depth: OrderDepth,
                                ...
                                avg_costs: Dict[str, float], memory: dict,
                                decoded_own_trades: Dict[str, List[OwnTrade]]) -> List[Order]:
        orders = []
        bug_tags = get_recent_bug_tags(product, decoded_own_trades)
        reverse_logic = "noise" in bug_tags or "risky" in bug_tags
        # Set default confidence multiplier before applying bug reactions
        if regime == "stable":
            confidence_mult = 2.15
        else:
            confidence_mult = 1.28

        fair_value, confidence_mult = self.apply_bug_reactions(fair_value, confidence_mult, product, decoded_own_trades, memory)
        if memory.get("skip_trade"):
            memory["skip_trade"] = False
            print(f"[SKIP TRADE] Avoiding illiquid/rare bug on {product}")
            return []
        if reverse_logic:
            print(f"Reversing logic on {product} due to bug behavior: {bug_tags}")
        ...
        for i in range(1, NUM_LEVELS + 1):
            ...
            for ask_price, ask_volume in sorted(order_depth.sell_orders.items()):
                if ask_price <= buy_level_price:
                    ...
                    if qty > 0:
                        orders.append(Order(product, ask_price, -qty if reverse_logic else qty))
        ...
        for i in range(1, NUM_LEVELS + 1):
            ...
            for bid_price, bid_volume in sorted(order_depth.buy_orders.items(), reverse=True):
                if bid_price >= sell_level_price:
                    ...
                    qty = min(int(confidence * confidence_mult), limit + current_position)
                    if qty > 0:
                        orders.append(Order(product, bid_price, qty if reverse_logic else -qty))
                        new_size = current_position - qty
                        avg_costs[product] = self.update_avg_cost(avg_costs.get(product, 0),
    ...
    def basket_arbitrage(self, basket_name: str, components: Dict[str, int],
                         ...
                         decoded_own_trades: Dict[str, List[OwnTrade]],
                         memory: dict) -> List[Order]:
        orders = []
        depth = state.order_depths[basket_name]
        position = state.position.get(basket_name, 0)
        bug_tags = get_recent_bug_tags(basket_name, decoded_own_trades)
        reverse_logic = "noise" in bug_tags or "risky" in bug_tags
        
        # 🧠 Define comp_value and confidence_mult BEFORE calling bug logic
        ...
        # Initial multiplier before adjustments
        confidence_mult = 2.0

        # 🐞 Apply bug-based tweaks
        comp_value, confidence_mult = self.apply_bug_reactions(comp_value, confidence_mult, basket_name, decoded_own_trades, memory)
        
        if memory.get("skip_trade"):
            memory["skip_trade"] = False
            print(f"[SKIP TRADE] Avoiding illiquid/rare bug on {basket_name}")
            return []
        if reverse_logic:
            print(f"[BASKET] Reversing logic on {basket_name} due to: {bug_tags}")
        ...
        # Momentum boost based on component trends
        avg_momentum = sum(
            (fair_values[p] - fair_values[p] * 0.98) * q
            for p, q in components.items()
        ) / sum(components.values())

        # Adaptive threshold
        base_threshold = max(3, int(comp_vol * 0.5))
        threshold = base_threshold * (0.9 if avg_momentum > 0.02 else 1.1 if avg_momentum < -0.02 else 1.0)

        # Dynamic confidence multiplier
        confidence_mult = 2.5 if avg_momentum > 0.04 else 1.5 if avg_momentum < -0.04 else 2.0
        ...
        for bid_price, bid_volume in sorted(depth.buy_orders.items(), reverse=True):
            if bid_price > comp_value + threshold:
                ...
                qty = min(int(confidence * confidence_mult), limit + position)
                if qty > 0:
        ...
        for ask_price, ask_volume in sorted(depth.sell_orders.items()):
            if ask_price < comp_value - threshold:
                ...
                qty = min(int(confidence * confidence_mult), limit - position)
                if qty > 0:
                    orders.append(Order(basket_name, ask_price, -qty if reverse_logic else qty))
                    position += qty
                if abs(position) >= limit:
                    break
    ...
    def apply_bug_reactions(self, fair_value: float, confidence_mult: float, product: str, decoded_own_trades: Dict[str, List[OwnTrade]], memory: dict):
        tags = []
        trades = decoded_own_trades.get(product, [])
        for t in reversed(trades[-5:]):
            if t.counter_party:
                tags += get_bug_tags(t.counter_party)

        if "mimic" in tags or "crowd" in tags:
            fair_value += 1 if memory.get("tick", 0) % 2 == 0 else -1

        if "noise" in tags or "emotional" in tags or "hoarder" in tags:
            confidence_mult *= 0.75

        if "momentum" in tags:
            confidence_mult *= 1.2

        if "illiquid" in tags or "rare" in tags:
            memory["skip_trade"] = True

        if "HFT" in tags or "front-runner" in tags:
            memory["skip_tick"] = True

        return fair_value, confidence_mult
