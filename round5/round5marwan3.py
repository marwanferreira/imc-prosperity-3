...
# --- BUG SIGNAL DETECTION ENGINE ---
class SignalEngine:
    def __init__(self):
        self.active_signals = {}
        self.PAIR_SIGNAL_MAP = {
            ("RAINFOREST_RESIN", "Penelope", "Charlie"): "long",
            ("RAINFOREST_RESIN", "Gary", "Charlie"): "long",
            ("RAINFOREST_RESIN", "Paris", "Charlie"): "long",
            ("RAINFOREST_RESIN", "Pablo", "Charlie"): "long",
            ("PICNIC_BASKET1", "Penelope", "Charlie"): "long",
            ("PICNIC_BASKET1", "Camilla", "Penelope"): "long",
        }

    def detect_signals(self, state: TradingState) -> dict:
        signals = {}
        for symbol, trades in state.market_trades.items():
            for t in trades[-5:]:  # check last 5 trades per product
                key = (symbol, t.seller, t.buyer)
                if key in self.PAIR_SIGNAL_MAP:
                    signals[symbol] = self.PAIR_SIGNAL_MAP[key]
    ...
    def get_signal(self, symbol: str) -> str | None:
...
class Trader:
    def run(self, state: TradingState):
        self.signal_engine = getattr(self, "signal_engine", SignalEngine())
        bug_signals = self.signal_engine.detect_signals(state)
        if bug_signals:
            print("🪲 ACTIVE BUG SIGNALS:", bug_signals)
        bug_signals["RAINFOREST_RESIN"] = "long"
        result = {}
        conversions = 0
        memory = {}
        if state.traderData:
        ...
        for product in [
            ...
            if product in state.order_depths:
                signal = bug_signals.get(product)
                if signal == "long":
                    fair_values[product] += 12
                    # ⬇️ Aggressive market buy inside same block
                    position = state.position.get(product, 0)
                    limit = POSITION_LIMITS.get(product, 50)
                    order_depth = state.order_depths[product]
                    best_ask = self.get_best_ask(order_depth)
                    if best_ask and position < limit:
                        result.setdefault(product, []).append(Order(product, best_ask, 3))
                elif signal == "short":
                    fair_values[product] -= 12
                    # ⬇️ Aggressive market sell inside same block
                    position = state.position.get(product, 0)
                    limit = POSITION_LIMITS.get(product, 50)
                    order_depth = state.order_depths[product]
                    best_bid = self.get_best_bid(order_depth)
                    if best_bid and position > -limit:
                        result.setdefault(product, []).append(Order(product, best_bid, -3))
        ...
        memory["active_bug_signals"] = bug_signals
